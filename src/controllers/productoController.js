...
const createProducto = async (req, res) => {
  try {
    const { nombre, descripcion, precio, imagenUrl, especificaciones, idCategoria, calificacion } = req.body;
    ...
    const newProducto = await Producto.create({ nombre, descripcion, precio, imagenUrl, especificaciones, idCategoria, calificacion });
    res.status(201).json({ status: 201, message: 'Se creó de manera exitosa un nuevo producto', newProducto });
  } catch (err) {
    ...
    res.status(500).json({ error: "Error al intentar crear un nuevo Producto", message: err.message });
  }
}
